...
export default function AdminOrders() {
  ...
  const [activeFilter, setActiveFilter] = useState<OrderStatus | "alla">(
    "alla",
  );
  ...
  const filtered = [...orders]
    .filter((order) =>
      activeFilter === "alla" ? true : order.status === activeFilter,
    )
  ...
  return (
    <div className="admin-orders">
      ...
      <div className="admin-filter-row">
        {(["alla", "pending", "delivered", "cancelled"] as const).map(
          (filter) => (
            <button
              key={filter}
            ...
            >
              {filter === "alla" ? "Alla" : statusLabels[filter]}
            </button>
          ),
        )}
      </div>
      ...
      <div className="admin-table-header">
        <span style={{ flex: 1 }}>
          Ordrar —{" "}
          {activeFilter === "alla" ? "Alla" : statusLabels[activeFilter]}
        </span>

        <div className="admin-sort-row">
          {(["az", "datum"] as SortOption[]).map((sort) => (
            <button
              key={sort}
  ...
  <>
    ...
    {item.selected_print_color.price > 0 && (
      <p>
        ...
        {filtered.length === 0 && <p>Inga ordrar hittades.</p>}
      </div>
    </div>
  );
}
