...
export default function Payment() {
  ...
  const { items, getTotalPrice, discountCode } = useCartStore();
  ...
  const validateForm = () => {
    ...
    if (!trimmedFirstName) {
      return language === "sv" ? "Fyll i förnamn." : "Enter your first name.";
    }
    ...
    if (!trimmedLastName) {
      return language === "sv" ? "Fyll i efternamn." : "Enter your last name.";
    }
    ...
    if (!trimmedAddress) {
      return language === "sv" ? "Fyll i adress." : "Enter your address.";
    }
    ...
    if (isSweden && !/^\d{3}\s?\d{2}$/.test(trimmedZipCode)) {
      return language === "sv"
    ...
    if (!trimmedCity) {
      return language === "sv" ? "Fyll i ort." : "Enter your city.";
    }
    ...
    if (!trimmedCountry) {
      return language === "sv" ? "Välj land." : "Select a country.";
    }
  ...
  const handleCreateOrder = async () => {
    if (items.length === 0) {
      setError(
        language === "sv" ? "Kundkorgen är tom." : "Your cart is empty.",
      );
    ...
    <div className="payment">
      <div className="payment-container">
        <div className="payment-header">
          <h1>{language === "sv" ? "Betalning" : "Checkout"}</h1>
        ...
        <div className="payment-layout">
          <div className="payment-form">
            <h2>{language === "sv" ? "Kunduppgifter" : "Customer details"}</h2>

            <div className="payment-form-grid">
              <div className="payment-field">
                <label>{language === "sv" ? "Förnamn" : "First name"}</label>
              ...
              <div className="payment-field">
                <label>{language === "sv" ? "Efternamn" : "Last name"}</label>
              ...
              <div className="payment-field full-width">
                <label>{language === "sv" ? "E-post" : "Email"}</label>
              ...
              <div className="payment-field full-width">
                <label>{language === "sv" ? "Adress" : "Address"}</label>
              ...
              <div className="payment-field">
                <label>{language === "sv" ? "Ort" : "City"}</label>
              ...
              <div className="payment-field full-width">
                <label>{language === "sv" ? "Land" : "Country"}</label>
                ...
                >
                  ...
                  <option value="NO">
                    {language === "sv" ? "Norge" : "Norway"}
                  </option>

                  <option value="DK">
                    {language === "sv" ? "Danmark" : "Denmark"}
                  </option>

                  <option value="FI">Finland</option>

                  <option value="IS">
                    {language === "sv" ? "Island" : "Iceland"}
                  </option>

                  <option value="DE">
                    {language === "sv" ? "Tyskland" : "Germany"}
                  </option>

                  <option value="NL">
                    {language === "sv" ? "Nederländerna" : "Netherlands"}
                  </option>
                </select>
              </div>
            </div>
          ...
          <div className="payment-summary">
            ...
            <div className="payment-summary-row">
              <span>{language === "sv" ? "Frakt" : "Shipping"}</span>
            ...
            <button
              ...
              disabled={loading || shippingLoading || shipping === null}
            >
              {loading
                ? language === "sv"
                  ? "Skapar order..."
